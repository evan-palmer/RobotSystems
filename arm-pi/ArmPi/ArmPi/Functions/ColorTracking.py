...
def move():
    ...
    while True:
        if __isRunning:
            if (
                first_move and start_pick_up
            ):  # when an object be detected for the first time
                action_finish = False
                ...
                time.sleep(
                    result[2] / 1000
                )  # the thrid item of return parameter is time
                start_pick_up = False
                first_move = False
                action_finish = True
            elif (
                not first_move and not unreachable
            ):  # not the first time to detected object
                set_rgb(detect_color)
